...
void IRAM_ATTR disp_spi_transfer_cmd(int8_t cmd) {
    ...
    gpio_set_level(tft_pins_global.dc_gpio, 0);
...
void IRAM_ATTR disp_spi_transfer_cmd_data(int8_t cmd, uint8_t *data, uint32_t len) {
	...
	gpio_set_level(tft_pins_global.dc_gpio, 1);
...
static void IRAM_ATTR _TFT_pushColorRep(color_t *color, uint32_t len, uint8_t rep, uint8_t wait)
{
	...
	gpio_set_level(tft_pins_global.dc_gpio, 1);								// Set DC to 1 (data mode);
...
void TFT_PinsInit()
{
    ...
    gpio_pad_select_gpio(tft_pins_global.cs_gpio);
    gpio_pad_select_gpio(tft_pins_global.miso_gpio);
    gpio_pad_select_gpio(tft_pins_global.mosi_gpio);
    gpio_pad_select_gpio(tft_pins_global.sclk_gpio);
    gpio_pad_select_gpio(tft_pins_global.dc_gpio);

    gpio_set_direction(tft_pins_global.miso_gpio, GPIO_MODE_INPUT);
    gpio_set_pull_mode(tft_pins_global.miso_gpio, GPIO_PULLUP_ONLY);
    gpio_set_direction(tft_pins_global.cs_gpio, GPIO_MODE_OUTPUT);
    gpio_set_direction(tft_pins_global.mosi_gpio, GPIO_MODE_OUTPUT);
    gpio_set_direction(tft_pins_global.sclk_gpio, GPIO_MODE_OUTPUT);
    gpio_set_direction(tft_pins_global.dc_gpio, GPIO_MODE_OUTPUT);
    gpio_set_level(tft_pins_global.dc_gpio, 0);

    gpio_pad_select_gpio(tft_pins_global.bckl_gpio);
    gpio_set_direction(tft_pins_global.bckl_gpio, GPIO_MODE_OUTPUT);
    gpio_set_level(tft_pins_global.bckl_gpio, tft_pins_global.bckl_on_value);

    gpio_pad_select_gpio(tft_pins_global.rst_gpio);
    gpio_set_direction(tft_pins_global.rst_gpio, GPIO_MODE_OUTPUT);
    gpio_set_level(tft_pins_global.rst_gpio, 0);
}
...
void TFT_display_init()
{
    ...
    gpio_set_level(tft_pins_global.rst_gpio, 1);
	...
	if(tft_pins_global.bckl_off_value)
    {
		gpio_set_level(tft_pins_global.rst_gpio, 0);
	}
	else
